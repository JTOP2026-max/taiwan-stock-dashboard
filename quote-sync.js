(function(){
  ...
  async function proxyQuote(code){if(!PROXY_URL)return null;try{const u=`${PROXY_URL.replace(/\/$/,'')}/quote?code=${encodeURIComponent(code)}&_=${Date.now()}`;const r=await fetch(u,{cache:'no-store'});if(!r.ok)return null;const q=await r.json();if(!q?.ok||!(num(q.price)>0))return null;return {price:num(q.price),prev:num(q.prev),ch:num(q.chg),pc:num(q.pct),open:num(q.open),high:num(q.high),low:num(q.low),name:q.name||'',time:q.time||'',date:q.date||'',source:q.source||'Cloudflare 行情代理',kind:'live-proxy'};}catch(e){return null}}
  async function misQuote(code){for(const market of ['tse','otc']){try{const u=`https://mis.twse.com.tw/stock/api/getStockInfo.jsp?ex_ch=${market}_${encodeURIComponent(code)}.tw&json=1&delay=0&_=${Date.now()}`;const r=await fetch(u,{cache:'no-store'});if(!r.ok)continue;const j=await r.json(),row=Array.isArray(j.msgArray)?j.msgArray.find(x=>String(x.c||'')===String(code)):null;if(!row)continue;const price=num(row.z),prev=num(row.y);if(!(price>0))continue;const ch=prev!=null?price-prev:0,pc=prev?ch/prev*100:0;return {price,prev,ch,pc,open:num(row.o),high:num(row.h),low:num(row.l),name:row.n||row.nf||'',time:row.t||'',date:row.d||'',source:'TWSE MIS 最新成交（瀏覽器備援）',kind:'live-direct'};}catch(e){}}return null}
  ...
  function loadEventRisk(){if(document.querySelector('script[data-event-risk]'))return;const s=document.createElement('script');s.src='event-risk-ui.js?v=20260826a';s.async=false;s.dataset.eventRisk='1';document.head.appendChild(s);}
  ...
  function start(){loadMarketStructure();loadEventRisk();loadPeScale();setTimeout(()=>syncHoldings(true),900);setInterval(()=>syncHoldings(false),SYNC_MS);document.addEventListener('visibilitychange',()=>{if(!document.hidden)syncHoldings(true)});window.addEventListener('focus',()=>syncHoldings(true));}
